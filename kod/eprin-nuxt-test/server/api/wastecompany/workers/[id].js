//Security risk: Uživatel může zobrazit jakékoliv id když to jednoduše obejde, použijte getOwnBranchInfo.js když jen pro sebe
export default defineEventHandler(async (event) => {
  const token = await handleToken(event);
  if (token == false) {
    console.log("FALSE");
    throw new Error("Nemáš přístupový token zkus se odhlásit a přihlásit!");
  }
  let id = await getRouterParam(event, "id");
  try {
